...
class HabitBlock {
  ...
  constructor(habitId, startDate) {
    ...
    this._endDate = this._startDate.plus({
      days: HabitBlock.#NUM_DAYS_IN_BLOCK,
    });
  ...
  get done() {
    return this._doneDays === HabitBlock.#NUM_DAYS_IN_BLOCK;
  }
  ...
  _makeActiveDays() {
    ...
    for (let date of dateGenerator(interval)) {
      ...
      activeDays.push(day);
      if (activeDays.length && activeDays[0].date.month !== date.month)
        this._numActiveFirstMonth--;
    }
    return activeDays;
  }
  ...
  _getOverlappedDays() {
    ...
    let afterFirstMonthEnd = Interval.after(endMonthStart, {
      days: numMissingFirstMonth,
    });
    let beforeSecondMonthStart = Interval.before(endMonthStart, {
      days: numMissingSecondMonth,
    });
  ...
  _getSurroundingDays() {
    ...
    startMonthStart = startMonthStart.minus({
      days: startMonthStart.weekday - 1,
    });

    // Find all days before the startDate and after the endDate
    // and insert them around the activedays as dummies
    let beforeStartDate = Interval.fromDateTimes(
      startMonthStart,
      this._startDate
    );
